...
expect_contains('wallet apply command cd prefix', $command, "'cd', self::OPERATOR_WEB_CWD, '&&', 'php', 'yaamp/yiic.php', 'badpoolguard', 'wallet-send-apply'", $failures);
...
expect_contains('full batch reconciled reported', $apply, "'full_batch_reconciled'=>true", $failures);
expect_contains('guarded DB completion label', $apply, "'db_mutations'=>'guarded_transaction_committed'", $failures);
expect_contains('payout rows marked completed reported', $apply, "'payout_rows_marked_completed'=>true", $failures);
...
expect_contains('do not retry flag', $apply, 'do_not_retry_wallet_send_apply', $failures);
expect_contains('txid included in failure', $apply, '\'txid\'=>$txid', $failures);
expect_contains('amount selected as exact DB string', $command, 'CAST(P.amount AS CHAR) AS amount', $failures);
