...
$packageCalls=array_values(array_filter($creditCalls,function($call){return $call[0]==='account-credit-clear-approval-package';}));delay_expect(count($packageCalls)===1&&in_array('--selected-earning-ids=11,12',$packageCalls[0][1],true),'override phase 5 package was not bound to durable earning IDs',$fail);
delay_expect(!in_array('wallet-send-apply',array_map(function($call){return $call[0];},$creditCalls),true),'override phase 5 invoked wallet behavior',$fail);
$normalCreditLedger=$creditLedger;$normalCreditLedger['phase_results'][0]['payment_delay_override_used']=false;$normalCreditCalls=array();$normalCreditExecutor=function($command,$args)use(&$normalCreditCalls,$creditExecutor){$normalCreditCalls[]=array($command,$args);return $creditExecutor($command,$args);};
$normalCredited=(new BadpoolPaymentBatchPhaseAdapter($guard,$normalCreditExecutor))->creditAccounts($normalCreditLedger,array());
$normalPackageCalls=array_values(array_filter($normalCreditCalls,function($call){return $call[0]==='account-credit-clear-approval-package';}));delay_expect($normalCredited['status']==='pass'&&count($normalPackageCalls)===1&&in_array('--selected-earning-ids=11,12',$normalPackageCalls[0][1],true),'normal post-delay phase 5 did not share the durable earning scope contract',$fail);
$scopeMethod=new ReflectionMethod('BadpoolPaymentBatchPhaseAdapter','packageWithinLedgerScope');$scopeMethod->setAccessible(true);$coin=array('id'=>1267);
$exactPackage=array('items'=>array('selected_earnings'=>array(array('earning_id'=>11,'account_id'=>9,'coin_id'=>1267),array('earning_id'=>12,'account_id'=>9,'coin_id'=>1267))));delay_expect($scopeMethod->invoke($creditAdapter,$exactPackage,$creditLedger,'credit',$coin)===true,'exact durable credit scope was rejected',$fail);
$badPackages=array('missing earning'=>array(array('earning_id'=>11,'account_id'=>9,'coin_id'=>1267)),'extra earning'=>array(array('earning_id'=>11,'account_id'=>9,'coin_id'=>1267),array('earning_id'=>12,'account_id'=>9,'coin_id'=>1267),array('earning_id'=>13,'account_id'=>9,'coin_id'=>1267)),'wrong account'=>array(array('earning_id'=>11,'account_id'=>10,'coin_id'=>1267),array('earning_id'=>12,'account_id'=>10,'coin_id'=>1267)),'wrong coin'=>array(array('earning_id'=>11,'account_id'=>9,'coin_id'=>1268),array('earning_id'=>12,'account_id'=>9,'coin_id'=>1268)),'malformed ID'=>array(array('earning_id'=>'11x','account_id'=>9,'coin_id'=>1267),array('earning_id'=>12,'account_id'=>9,'coin_id'=>1267)),'duplicate ID'=>array(array('earning_id'=>11,'account_id'=>9,'coin_id'=>1267),array('earning_id'=>11,'account_id'=>9,'coin_id'=>1267)),'empty package'=>array());
