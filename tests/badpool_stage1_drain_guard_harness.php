...
expect_contains('base emits apply not executed false', $base, "\$report['apply_commands_executed'] = false;", $failures);
expect_contains('base emits projected earnings total', $base, 'total_projected_earnings', $failures);
expect_contains('base emits inserted earnings total', $base, 'total_inserted_earnings', $failures);
...
expect_contains('per batch inserted earnings defaults zero', $command, "'inserted_earnings_count'=>0", $failures);
expect_contains('apply flips executed flag before mutation', $apply, "\$report['apply_commands_executed'] = true;", $failures);
...
expect_contains('totals projected earnings from per batch', $command, "\$report['total_projected_earnings'] += intval(arraySafeVal(\$batch, 'projected_earnings_rows', 0));", $failures);
expect_contains('totals actual inserted from per batch', $command, "\$report['total_inserted_earnings'] += intval(arraySafeVal(\$batch, 'inserted_earnings_count', 0));", $failures);
