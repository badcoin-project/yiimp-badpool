...
class FixtureStore implements BadpoolLiveBlockStore {
	public $blocks=array(),$candidates=array(),$earnings=array(),$mutations=array(),$raceOnApply=false,$selectionCalls=0;
	public function candidates($coin,$algo,$after,$limit){$this->selectionCalls++;$out=array();foreach($this->candidates as $c){$b=$this->blocks[$c['block_id']];if($c['coin_id']===$coin&&$c['algo']===$algo&&$c['block_id']>$after&&$b['category']==='new'&&!isset($this->earnings[$c['block_id']]))$out[]=$c;}usort($out,function($a,$b){return $a['block_id']-$b['block_id'];});return array_slice($out,0,$limit);}
	public function apply($c,$x){$id=$c['block_id'];if($this->raceOnApply)$this->blocks[$id]['category']='generate';$b=&$this->blocks[$id];if($b['id']!==$id||$b['coin_id']!==$c['coin_id']||$b['blockhash']!==$c['blockhash']||$b['category']!=='new'||isset($this->earnings[$id]))return array('status'=>'skipped','reason'=>'no_longer_eligible');$before=$this->blocks;if($x['category']==='orphan'){$b['category']='orphan';$b['amount']=0;}else{if(empty($c['attribution'])){$this->blocks=$before;return array('status'=>'failed','reason'=>'transaction_exception');}$b=array_merge($b,$x);$this->earnings[$id]=array('status'=>0,'attribution'=>$c['attribution']);}$this->mutations[]=$id;return array('status'=>'applied');}
}
...
$boundary=new FixtureStore;$boundaryDaemon=new FixtureDaemon;
foreach(array(30000,30001,30002) as $id){$boundary->blocks[$id]=array('id'=>$id,'coin_id'=>1267,'blockhash'=>'h'.$id,'category'=>'new');$boundary->candidates[]=candidate($id);$boundaryDaemon->answers[$id]=array('category'=>'immature','txhash'=>'tx'.$id,'amount'=>8,'confirmations'=>2);}
$boundaryResult=(new BadpoolLiveBlockAccounting($boundary,$boundaryDaemon))->run(1267,'scrypt',30000,2);
ok($boundary->mutations===array(30001,30002)&&$boundaryResult['selected']===2,'exclusive boundary selects 30001 and 30002 in ascending order');
ok($boundary->blocks[30000]['category']==='new'&&!isset($boundary->earnings[30000]),'accumulated candidate at boundary remains untouched');
...
$s->blocks[30000]=array('id'=>30000,'coin_id'=>1267,'blockhash'=>'h30000','category'=>'new');$s->candidates[]=candidate(30000);$d->answers[30000]=array('category'=>'immature','txhash'=>'old','amount'=>8,'confirmations'=>2);
...
$r=(new BadpoolLiveBlockAccounting($s,$d))->run(1267,'scrypt',30000,2);
...
ok($s->blocks[30000]['category']==='new'&&!isset($s->earnings[30000]),'pre-activation live candidate excluded');
foreach(array(null,'',0,'0',-1,'-1','1x',1.5) as $bad){$calls=$s->selectionCalls;try{(new BadpoolLiveBlockAccounting($s,$d))->run(1267,'scrypt',$bad,2);ok(false,'invalid boundary refused');}catch(InvalidArgumentException $e){ok($s->selectionCalls===$calls,'invalid boundary refused before selection');}}
foreach(array(0,11) as $badLimit){try{(new BadpoolLiveBlockAccounting($s,$d))->run(1267,'scrypt',30000,$badLimit);ok(false,'limit refused');}catch(InvalidArgumentException $e){ok(true,'limit refused');}}
$s->blocks[30002]=array('id'=>30002,'coin_id'=>1267,'blockhash'=>'h30002','category'=>'new');$s->candidates[]=candidate(30002);$d->answers[30002]=array('category'=>'orphan','amount'=>0,'txhash'=>null,'confirmations'=>0);(new BadpoolLiveBlockAccounting($s,$d))->run(1267,'scrypt',30001,1);ok($s->blocks[30002]['category']==='orphan'&&!isset($s->earnings[30002]),'orphan no earning');
$s->blocks[30003]=array('id'=>30003,'coin_id'=>1267,'blockhash'=>'h30003','category'=>'new');$s->candidates[]=candidate(30003);$s->earnings[30003]=array('status'=>0);$d->answers[30003]=array('category'=>'immature');(new BadpoolLiveBlockAccounting($s,$d))->run(1267,'scrypt',30002,10);ok($s->blocks[30003]['category']==='new','existing earning excluded');
$s->blocks[30004]=array('id'=>30004,'coin_id'=>1267,'blockhash'=>'h30004','category'=>'new');$s->candidates[]=candidate(30004);$d->answers[30004]='fail';$before=$s->blocks[30004];$r=(new BadpoolLiveBlockAccounting($s,$d))->run(1267,'scrypt',30003,10);ok($s->blocks[30004]===$before&&$r['daemon_failed']===1&&!empty($r['daemon_failures']),'daemon failure atomic and explicit');
$race=new FixtureStore;$race->blocks[4]=array('id'=>4,'coin_id'=>1267,'blockhash'=>'h4','category'=>'new');$race->candidates[]=candidate(4);$race->raceOnApply=true;$d2=new FixtureDaemon;$d2->answers[4]=array('category'=>'immature','amount'=>1,'txhash'=>'t','confirmations'=>1);$r=(new BadpoolLiveBlockAccounting($race,$d2))->run(1267,'scrypt',3,1);ok(empty($race->mutations)&&!isset($race->earnings[4])&&$r['skipped']===1&&$r['apply_failed']===0,'category race is benign skip');
$empty=new FixtureStore;$empty->blocks[5]=array('id'=>5,'coin_id'=>1267,'blockhash'=>'h5','category'=>'new');$c=candidate(5);$c['attribution']=array();$empty->candidates[]=$c;$d2->answers[5]=array('category'=>'immature','amount'=>1,'txhash'=>'t','confirmations'=>1);$before=$empty->blocks[5];$r=(new BadpoolLiveBlockAccounting($empty,$d2))->run(1267,'scrypt',4,1);ok($empty->blocks[5]===$before&&!isset($empty->earnings[5])&&$r['apply_failed']===1&&!empty($r['apply_failures']),'atomic rollback and explicit apply failure');
...
ok(strpos($src,'C.block_id > :after')!==false,'exclusive activation boundary enforced in SQL');
