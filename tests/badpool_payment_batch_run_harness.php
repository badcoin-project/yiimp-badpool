...
expect_batch(count($mixedApplyCalls)===1,'mixed payout packages invoked an unexpected number of payout-row applies',$fail);
expect_batch(count($mixedApplyCalls)===1&&in_array('--selected-account-ids=79',$mixedApplyCalls[0][1],true)&&in_array('--approval-package-checksum='.str_repeat('c',64),$mixedApplyCalls[0][1],true),'mixed payout packages did not preserve the non-empty coin 1267/account 79 apply',$fail);
$emptySelectedAccountArg=false;foreach($mixedApplyCalls as $call)if(in_array('--selected-account-ids=',$call[1],true))$emptySelectedAccountArg=true;
