...
foreach(array('uncommissioned-yescrypt','uncommissioned-skein','live-groestl-v1','uncommissioned-sha256d') as $id){$lane=$registry->get($id);try{(new LiveBlockMaturityCommand())->actionIndex($lane->coinId(),$lane->dbAlgo(),$lane->blockBoundary()?:1,1,$id);inv_ok(false,$id.' command executed');}catch(InvalidArgumentException $e){}}
