...
$registry=new BadpoolLivePaymentLaneRegistry();foreach(array('uncommissioned-yescrypt','uncommissioned-skein','uncommissioned-sha256d') as $id){$lane=$registry->get($id);try{(new BadpoolLiveBlockMaturity(new class implements BadpoolLiveBlockMaturityStore{function candidates($c,$a,$b,$l){throw new RuntimeException('executed');}function apply($c,$r){throw new RuntimeException('executed');}},new class implements BadpoolLiveBlockMaturityDaemon{function inspect($c){throw new RuntimeException('executed');}},$lane))->run($lane->coinId(),$lane->dbAlgo(),$lane->blockBoundary()?:1,1);inv_ok(false,$id.' maturity executed');}catch(InvalidArgumentException $e){}}
$groestl=$registry->get('live-groestl-v1');$probe=new class implements BadpoolLiveBlockMaturityStore{public $calls=array();function candidates($c,$a,$b,$l){$this->calls[]=array($c,$a,$b,$l);return array();}function apply($c,$r){throw new RuntimeException('executed');}};$daemon=new class implements BadpoolLiveBlockMaturityDaemon{public $calls=0;function inspect($c){$this->calls++;throw new RuntimeException('executed');}};$result=(new BadpoolLiveBlockMaturity($probe,$daemon,$groestl))->run(1269,'badcoin-groestl',31212,2);inv_ok($probe->calls===array(array(1269,'badcoin-groestl',31212,2))&&$daemon->calls===0&&$result['selected']===0,'Groestl maturity construction did not accept the exact commissioned scope');
foreach(array(array(1268,'badcoin-groestl',31212,2),array(1269,'groestl',31212,2),array(1269,'badcoin-groestl',31211,2),array(1269,'badcoin-groestl',31212,11)) as $args){try{(new BadpoolLiveBlockMaturity($probe,$daemon,$groestl))->run($args[0],$args[1],$args[2],$args[3]);inv_ok(false,'Groestl maturity accepted mismatched identity, boundary, or limit');}catch(InvalidArgumentException $e){}}
inv_ok($registry->get('live-groestl-v1')->operationalAlgo()==='groestl'&&$registry->get('live-groestl-v1')->dbAlgo()==='badcoin-groestl','Groestl mapping changed');
...
$validated=LiveBlockMaturityCommand::configurationForRequest(1269,'badcoin-groestl',31212,2,'live-groestl-v1');inv_ok($validated->toArray()===$groestl->toArray(),'Groestl maturity command validation rejected the production command shape');
foreach(array('uncommissioned-yescrypt','uncommissioned-skein','uncommissioned-sha256d') as $id){$lane=$registry->get($id);try{LiveBlockMaturityCommand::configurationForRequest($lane->coinId(),$lane->dbAlgo(),$lane->blockBoundary()?:1,1,$id);inv_ok(false,$id.' command validation accepted maturity');}catch(InvalidArgumentException $e){}}
foreach(array(array(1268,'badcoin-groestl',31212,2),array(1269,'groestl',31212,2),array(1269,'badcoin-groestl',31211,2),array(1269,'badcoin-groestl',31212,11)) as $args){try{LiveBlockMaturityCommand::configurationForRequest($args[0],$args[1],$args[2],$args[3],'live-groestl-v1');inv_ok(false,'Groestl command validation accepted mismatched identity, boundary, or limit');}catch(InvalidArgumentException $e){}}
