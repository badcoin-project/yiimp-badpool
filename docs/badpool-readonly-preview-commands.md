...
`forward-catchup-preview` maps the forward catch-up window from the latest completed payout checkpoint for one coin. It reports coin identity, the `payouts.MAX(time)` checkpoint, post-checkpoint blocks grouped by category, new-block import candidates, a small read-only daemon sample for oldest candidates, blocked projected stages, safety classification, audit metadata, and checksum metadata. The daemon sample only reads `getblock` and `gettransaction` through the guarded wallet RPC wrapper; it does not send coins, create payout rows, change account balances, run backend loops, change rows, or add an approval package.

`forward-catchup-approval-package` generates an auditable, read-only approval package for future Stage 1 forward catch-up import review. It reuses the forward-catch-up preview model, reports preview dependency checksums, the Stage 1 approval package identity, proposed single-coin mutation scope, candidate summary, first batch plan, daemon sample summary, exact future apply intent as blocked, blocked later stages, safety metadata, `approval_input_checksum`, `intended_mutation_scope_checksum`, and `report_checksum`. It does not add an apply command, accept execute flags, write database rows, send coins, create payouts, change balances, run services, run cron, or run backend loops.

`forward-catchup-stage1-apply-preflight` verifies that the Stage 1 approval package inputs are internally consistent and that a future batch-limited Stage 1 apply design would have a safe boundary to build next. Optional `--approval-input-checksum=<checksum>` and `--mutation-scope-checksum=<checksum>` inputs are compared against recomputed package-equivalent checksums. The preflight reports dependency checks, candidate set counts, first batch boundary, read-only daemon verification, safety gates, future apply constraints, blocked later stages, audit metadata, `preflight_input_checksum`, `batch_scope_checksum`, and `report_checksum`. It is read-only and does not write database rows, send coins, create payouts, change balances, run services, run cron, or run backend loops.
