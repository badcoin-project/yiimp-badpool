...
* Plan is read-only, reports `apply_commands_executed=false`, reports `total_inserted_earnings=0`, and does not execute apply logic. Plan projections use `total_projected_earnings` and per-batch `projected_earnings_rows`.
