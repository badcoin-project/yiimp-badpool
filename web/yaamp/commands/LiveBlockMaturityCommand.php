...
class LiveBlockMaturityCommand extends CConsoleCommand
{
	...
	public function actionIndex($coin,$algo,$after,$limit,$lane='live-scrypt-v1')
	{
		...
		if(intval($coin)!==$config->coinId()||(string)$algo!==$config->dbAlgo()||intval($after)!==$config->blockBoundary())throw new InvalidArgumentException('coin, DB algo, and boundary do not match the selected lane');
