...
class BadpoolGuardCommand extends CConsoleCommand
{
	...
	public function getHelp()
	{
		return "Yiimp badpoolguard command\n".
			...
			"       php yaamp/yiic.php badpoolguard earnings-maturity-transition-apply --coin-id=<id> --approval-package-checksum=<sha256> --selected-scope-checksum=<sha256> --projected-block-mutation-checksum=<sha256> --projected-earnings-mutation-checksum=<sha256> --operator-confirms-maturity-transition=scrypt_status0_to_status1 --format=json\n".
			...
			"       php yaamp/yiic.php badpoolguard account-credit-clear-apply --coin-id=<id> --approval-package-checksum=<sha256> --selected-earnings-scope-checksum=<sha256> --projected-earnings-mutation-checksum=<sha256> --projected-account-credit-checksum=<sha256> --operator-confirms-account-credit=scrypt_status1_to_status2_balance_increment --format=json\n".
	...
	private function earningsMaturityTransitionDryrunReport()
	{
		...
		$rows = $this->guard->selectAll("SELECT E.id AS earning_id,E.userid,E.coinid,E.blockid,E.amount,E.status,E.mature_time,B.id AS block_id,B.height AS block_height,B.coin_id AS block_coin_id,B.category AS block_category FROM earnings E INNER JOIN blocks B ON B.id=E.blockid WHERE E.status=0 AND B.coin_id=:coin_id AND E.coinid=:coin_id AND B.category='immature' ORDER BY B.height,E.id", array(':coin_id'=>$coinId));
		$items = array(); $blocks = array(); $totalsByUser = array(); $total = 0.0; $heights = array();
		foreach ($rows as $r) {
			$item = array('earning_id'=>intval($r['earning_id']),'linked_block_id'=>intval($r['block_id']),'block_height'=>intval($r['block_height']),'userid'=>intval($r['userid']),'coinid'=>intval($r['coinid']),'amount'=>$this->decimalString($r['amount']),'current_earning_status'=>intval($r['status']),'current_block_category'=>$r['block_category'],'projected_block_category'=>'generate','projected_earnings_status'=>1,'projected_mature_time_behavior'=>'set_to_current_unix_timestamp_at_apply');
			$items[] = $item; $blocks[intval($r['block_id'])] = array('block_id'=>intval($r['block_id']),'height'=>intval($r['block_height']),'coin_id'=>intval($r['block_coin_id']),'from_category'=>$r['block_category'],'to_category'=>'generate');
		...
		$report = $this->guard->baseReport();
		$report['summary']['selection_criteria'] = array("earnings.status=0", "earnings.blockid=blocks.id", "blocks.coin_id=--coin-id", "blocks.category='immature'");
	...
	private function accountCreditClearDryrunReport()
	{
		...
		foreach($rows as $r){ $coin=getdbo('db_coins',intval($r['coinid'])); $user=getdbo('db_accounts',intval($r['userid'])); $value=($coin&&$user)?yaamp_convert_amount_user($coin,$r['amount'],$user):0; $item=array('earning_id'=>intval($r['earning_id']),'userid'=>intval($r['userid']),'coinid'=>intval($r['coinid']),'blockid'=>intval($r['blockid']),'amount'=>$this->decimalString($r['amount']),'mature_time'=>intval($r['mature_time']),'coin_price'=>$this->decimalString($r['coin_price']),'projected_converted_credit_value'=>$this->decimalString($value),'account_id'=>intval($r['account_id']),'account_coinid'=>intval($r['account_coinid']),'current_account_balance'=>$this->decimalString($r['account_balance']),'projected_account_balance'=>$this->decimalString(floatval($r['account_balance'])+$value)); $items[]=$item; $u=(string)$r['userid']; if(!isset($totals[$u]))$totals[$u]=array('userid'=>intval($r['userid']),'earning_count'=>0,'projected_credit_total'=>'0'); $totals[$u]['earning_count']++; $totals[$u]['projected_credit_total']=$this->decimalAdd($totals[$u]['projected_credit_total'],$value); $creditTotal+=$value; }
	...
	private function guardedApplyReport($args, $type)
	{
		...
		$required = $type==='earnings-maturity-transition' ? array('approval-package-checksum','selected-scope-checksum','projected-block-mutation-checksum','projected-earnings-mutation-checksum') : array('approval-package-checksum','selected-earnings-scope-checksum','projected-earnings-mutation-checksum','projected-account-credit-checksum');
		...
		$approval = $type==='earnings-maturity-transition' ? $this->earningsMaturityTransitionApprovalPackageReport() : $this->accountCreditClearApprovalPackageReport(); if(!$this->guard->isValid()) return $this->guardedApplyFail($report,'approval_package_recompute_failed','Fresh approval package recompute failed.');
	...
	private function parseGuardedApplyOptions($args){ $allowed=array('coin-id','format','approval-package-checksum','selected-scope-checksum','projected-block-mutation-checksum','projected-earnings-mutation-checksum','operator-confirms-maturity-transition','selected-earnings-scope-checksum','projected-account-credit-checksum','operator-confirms-account-credit'); $o=array(); foreach($args as $arg){ if(!preg_match('/^--([^=]+)=(.*)$/',$arg,$m)){ $o['__parse_error']='Unknown argument refused: '.$arg; continue; } $n=strtolower($m[1]); if(!in_array($n,$allowed,true)) $o['__parse_error']='Unknown option refused: --'.$m[1]; elseif(isset($o[$n])) $o['__parse_error']='Duplicate option refused: --'.$m[1]; else $o[$n]=$m[2]; } return $o; }
