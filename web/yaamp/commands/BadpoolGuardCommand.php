...
class BadpoolGuardCommand extends CConsoleCommand
{
	...
	private function earningsMaturityTransitionDryrunReport($args=array())
	{
		...
		foreach ($rows as $r) {
			...
			$item = array('earning_id'=>intval($r['earning_id']),'linked_block_id'=>$blockId,'block_height'=>intval($r['block_height']),'userid'=>intval($r['userid']),'coinid'=>intval($r['coinid']),'amount'=>BadpoolStage1Manifest::normalizeAmount($r['amount']),'current_earning_status'=>intval($r['status']),'current_earning_mature_time'=>intval($r['mature_time']),'current_block_category'=>$r['block_category'],'block_confirmations'=>intval($r['confirmations']),'coin_mature_blocks'=>intval($r['mature_blocks']),'maturity_proof_status'=>$proof['status'],'maturity_proof_reason'=>$proof['reason'],'projected_block_category'=>'generate','projected_earnings_status'=>1,'projected_mature_time_behavior'=>'set_to_current_unix_timestamp_at_apply');
			$items[] = $item;
