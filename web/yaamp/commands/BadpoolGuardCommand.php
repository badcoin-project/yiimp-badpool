...
class BadpoolGuardCommand extends CConsoleCommand
{
	...
	private function forwardCatchupStage1ApplyApprovalPackageReport()
	{
		...
		$metadata = $this->forwardCatchupStage1ApprovalMetadata();
		$intendedMutationScope = $this->forwardCatchupStage1ApprovalMutationScope();
		$futureApplyGates = $this->forwardCatchupStage1ApprovalFutureGates();
		$approvalInput = array(
			'metadata' => $metadata,
			'dryrun_output_summary' => $totals,
			'dryrun_checksums' => $dryrunChecksums,
			'intended_mutation_scope' => $intendedMutationScope,
			'future_apply_gates' => $futureApplyGates,
		);
		$approvalInputChecksum = BadpoolGuardReport::checksum($approvalInput);
		$intendedMutationScopeChecksum = BadpoolGuardReport::checksum($intendedMutationScope);
		$approvalPackageChecksum = BadpoolGuardReport::checksum(array(
			'approval_input_checksum' => $approvalInputChecksum,
			'intended_mutation_scope_checksum' => $intendedMutationScopeChecksum,
			'dryrun_checksums' => $dryrunChecksums,
		));
		...
		$report['payout_rows_created'] = false;
		$report['backend_loops_run'] = false;
		$report['approval_package_type'] = arraySafeVal($metadata, 'approval_package_type');
		$report['approval_required'] = true;
		$report['apply_command_not_implemented'] = true;
		$report['approval_package_checksum'] = $approvalPackageChecksum;
		$report['approval_input_checksum'] = $approvalInputChecksum;
		$report['intended_mutation_scope_checksum'] = $intendedMutationScopeChecksum;
		$report['dryrun_input_checksum'] = arraySafeVal($dryrunChecksums, 'dryrun_input_checksum');
		$report['batch_scope_checksum'] = arraySafeVal($dryrunChecksums, 'batch_scope_checksum');
		$report['projected_mutation_checksum'] = arraySafeVal($dryrunChecksums, 'projected_mutation_checksum');
		$report['projected_earnings_checksum'] = arraySafeVal($dryrunChecksums, 'projected_earnings_checksum');
		$report['summary']['approval_package_metadata'] = $metadata;
		$report['summary']['dryrun_output_summary'] = $totals;
		...
		$report['summary']['mandatory_future_apply_gates'] = $futureApplyGates;
		$report['summary']['safety_gates'] = $this->forwardCatchupStage1ApprovalSafetyGates($dryrun);
		$report['summary']['recommended_next_stage'] = 'forward-catchup-stage1-apply-command-design';
		$report['summary']['selected_count'] = arraySafeVal($totals, 'selected_count');
		$report['summary']['first_height'] = arraySafeVal($totals, 'first_height');
		$report['summary']['last_height'] = arraySafeVal($totals, 'last_height');
		$report['items']['dryrun_report'] = $dryrun;
	...
	private function forwardCatchupStage1ApprovalMetadata()
	{
		return array(
	...
	private function forwardCatchupStage1ApprovalMutationScope()
	{
		return array(
			'stage1_import_generate' => array(
				'update_block_fields' => array('txhash', 'amount', 'confirmations', "category='immature'"),
				'insert_pending_earning' => true,
				'earning_status' => 0,
			),
			...
			'stage1_mark_orphan_no_earnings' => array(
				'update_block_fields' => array("category='orphan'"),
				'insert_pending_earning' => false,
				'earning_status' => null,
			),
	...
	private function forwardCatchupStage1ApprovalFutureGates()
	{
		return array(
	...
	private function forwardCatchupStage1ApprovalSafetyGates($dryrun)
	{
		$dryrunGates = arraySafeVal(arraySafeVal($dryrun, 'summary', array()), 'safety_gates', array());
		$dryrunStatusPass = arraySafeVal($dryrunGates, 'overall_dryrun_status') === 'pass';
		$gates = array(
			'gate_read_only' => true,
			...
			'gate_no_wallet_sends' => true,
			'gate_no_backend_loop_calls' => true,
			'gate_dryrun_status_pass' => $dryrunStatusPass,
