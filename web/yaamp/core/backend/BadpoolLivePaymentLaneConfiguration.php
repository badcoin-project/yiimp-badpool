...
class BadpoolLivePaymentLaneConfiguration
{
	...
	public function isAccountingCommissioned() { return $this->get('accounting_enabled')===true; }
	public function isMaturityCommissioned() { return $this->get('maturity_enabled')===true; }
	public function isPayoutPreparationCommissioned() { return $this->get('payout_preparation_enabled')===true; }
	public function isWalletSendCommissioned() { return $this->get('wallet_send_enabled')===true; }
	/** Compatibility alias: commissioned means commissioned for payout preparation. */
	public function isCommissioned() { return $this->isPayoutPreparationCommissioned(); }
	...
	private function validate()
	{
		...
		if ($v['maturity_enabled'] && !$v['accounting_enabled']) throw new InvalidArgumentException('Maturity requires commissioned accounting.');
		if ($v['payout_preparation_enabled'] && (!$v['accounting_enabled'] || !$v['maturity_enabled'])) throw new InvalidArgumentException('Payout preparation requires commissioned accounting and maturity.');
		if ($v['wallet_send_enabled'] && !$v['payout_preparation_enabled']) throw new InvalidArgumentException('Wallet send requires commissioned payout preparation.');
		$this->validateStageValue($v,'accounting_enabled','block_id_gt','Accounting-enabled lanes require a positive block boundary.','Accounting-disabled lanes must not imply a block boundary.');
		$this->validateStageValue($v,'maturity_enabled','maturity_max_blocks','Maturity-enabled lanes require a positive block limit.','Maturity-disabled lanes must not imply a block limit.');
		$this->validateStageValue($v,'payout_preparation_enabled','batch_max_earnings','Payout-preparation-enabled lanes require a positive batch limit.','Payout-preparation-disabled lanes must not imply a batch limit.');
	...
	private function validateStageValue($values,$enabledKey,$valueKey,$enabledError,$disabledError)
	{
		$present=array_key_exists($valueKey,$values)&&$values[$valueKey]!==null;
		if($values[$enabledKey]&&(!$present||!is_int($values[$valueKey])||$values[$valueKey]<1))throw new InvalidArgumentException($enabledError);
		if(!$values[$enabledKey]&&$present)throw new InvalidArgumentException($disabledError);
	}
}
...
class BadpoolLivePaymentLaneRegistry
{
	...
	public function fromOwnershipEnvelope($owner) { if(!is_array($owner)||!isset($owner['lane']))return null; try{$lane=$this->get($owner['lane']);}catch(Exception $e){return null;} return $lane->isPayoutPreparationCommissioned()&&$lane->ownershipMatches($owner)?$lane:null; }
	...
	private static function configuredLanes()
	{
		...
		$groestl=array_merge($base,array('lane_id'=>'live-groestl-v1','coin_id'=>1269,'db_algo'=>'badcoin-groestl','operational_algo'=>'groestl','block_id_gt'=>31212,'batch_max_earnings'=>null,'maturity_max_blocks'=>null,'state_filename'=>'live-groestl-coordinator.json','lock_filename'=>'live-groestl-coordinator.lock','wallet_binding_identity'=>'groestl','wallet_source_account'=>'pool-groestl','accounting_enabled'=>true,'maturity_enabled'=>false,'payout_preparation_enabled'=>false,'wallet_send_enabled'=>false));
		$disabled=array(
		...
		$out=array(new BadpoolLivePaymentLaneConfiguration($enabled),new BadpoolLivePaymentLaneConfiguration($groestl));
