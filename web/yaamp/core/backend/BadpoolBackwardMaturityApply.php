...
class BadpoolBackwardMaturityApply
{
	...
	const APPROVAL_FILE_SHA256 = 'acc0955d8b97720f86a18fe7e2308d3007157ab2dcea43c66f143d1099a27b7a';
	...
	{
		return 'I confirm coin_id 1267 row_count 71 amount 153624.877229050000 approval_package_file_sha256 '.$approvalChecksum.'; perform only earnings status maturity and block category maturity transition; no account credit, payout, wallet, service, backend loop, or share deletion';
	}
	...
	public static function run($args,$store,$expectedBindings=array())
	{
		...
		if(isset($expectedBindings['approval_package_file_sha256'])&&!hash_equals($expectedBindings['approval_package_file_sha256'],$o['approval-package-checksum']))return self::hold($report,'Approval package is not the deployed reviewed file binding.');
		...
		$checks=self::packageChecks($approval,$dry,$o,$p['earning_ids'],$p['block_ids']);$report['pre_apply_validation']['retained_package_checks']=$checks;
	...
	private static function packageChecks($a,$d,$o,$eids,$bids)
	{
		...
		return array(
			...
			'internal_checksum_valid'=>self::v($checksum,'value')===BadpoolGuardReport::checksum($copy)['value'],'checksum_non_authorizing'=>self::v($checksum,'purpose')===BadpoolBackwardMaturityApprovalPackage::CHECKSUM_PURPOSE,'no_embedded_apply'=>!isset($a['apply_command'])&&!isset($a['apply_command_args'])&&!isset($a['apply_command_shape']),'blocked_actions'=>$allBlocked,'review_binding_only'=>strpos(self::v($checksum,'purpose',''),'not apply authorization')!==false,
