...
class BadpoolLivePaymentCoordinator
{
	...
	private function runLocked() {
		...
		if($id!==null&&(count($owned)!==1||!isset($owned[$id])))return $this->report('FAIL_CLOSED',$id,null,'reconciled_state',array('Coordinator state and owned batch evidence disagree.'));
		if($id===null&&count($owned)===1){$id=key($owned);$action='reconciled_state';}else $action='none';
		if($id!==null){
			if(!isset($owned[$id]))return $this->report('FAIL_CLOSED',$id,null,'reconciled_state',array('Active ledger missing, malformed, mismatched, or not coordinator-owned.'));
			$l=$owned[$id];$error=$this->validate($l,$id);if($error)return $this->report('FAIL_CLOSED',$id,$l,'reconciled_state',array($error));
			$c=$this->classify($l);
			if($c==='READY_FOR_NEW_BATCH'){$this->saveState(null,$l,$c,$id);$id=null;$action='reconciled_state';}
		...
		if(count($l['selected_earning_ids'])===0){$this->removeEmpty($id,$l);$this->saveState(null,null,'IDLE',null);return $this->report('IDLE',null,null,'none',array());}
	...
	private function removeEmpty($id,$l){if(count($l['selected_earning_ids'])===0&&count((array)@$l['created_payout_ids'])===0){@unlink($this->root.'/'.$id.'/ledger.json');@rmdir($this->root.'/'.$id);}}
