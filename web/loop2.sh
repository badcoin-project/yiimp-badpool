...
refuse() {
	...
	printf '{"schema":"badpool.backend-cycle.v2","route":"loop2","mode":"invalid","started_at_utc":"%s","completed_at_utc":"%s","lock_acquired":false,"readiness_gate":"refused","callbacks_started":[],"callbacks_completed":[],"callbacks_failed":[],"declared_effect_classes":[],"instrumentation_available":false,"result":"refused","errors":["%s"]}\n' "$now" "$now" "$message"
	exit "$code"
}
if ! cd "${DIR}"; then
